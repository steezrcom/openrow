...
func (s *Service) Create(ctx context.Context, tenantID, schema string, spec *EntitySpec) (*Entity, error) {
	...
	var entityID string
	err = tx.QueryRow(ctx, `
		INSERT INTO openrow.entities (tenant_id, name, display_name, description)
		VALUES ($1, $2, $3, $4)
		RETURNING id`,
...
func (s *Service) AddField(ctx context.Context, tenantID, schema string, entityName string, f FieldSpec) (*Entity, error) {
	...
	if _, err := tx.Exec(ctx, `UPDATE openrow.entities SET updated_at = now() WHERE id = $1`, ent.ID); err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, err
...
func (s *Service) DropField(ctx context.Context, tenantID, schema, entityName, fieldName string) error {
	...
	if _, err := tx.Exec(ctx, `DELETE FROM openrow.fields WHERE id = $1`, fieldID); err != nil {
		return err
	}
...
func lookupEntityID(ctx context.Context, q querier, tenantID, name string) (string, error) {
	var id string
	err := q.QueryRow(ctx,
		`SELECT id FROM openrow.entities WHERE tenant_id = $1 AND name = $2`,
		tenantID, name,
	).Scan(&id)
